...
function calculateSimilarity(str1: string, str2: string): number {
  ...
  return Math.max(
    levenshteinScore,
    keywordScore,
    (levenshteinScore + keywordScore) / 2,
  );
}
...
function calculateMatchQuality(
  ...
  const lengthBonus =
    normalizedItem.length >= normalizedSearch.length ? 0.05 : 0;

  return (
    baseSimilarity * 0.7 + keywordCoverage * 0.2 + fullMatchBonus + lengthBonus
  );
}
...
export function findBestMatch(
  ...
  console.log(
    '🔍 [FUZZY SEARCH] Palavras-chave extraídas:',
    getKeywords(searchName),
  );
  console.log('🔍 [FUZZY SEARCH] Threshold:', threshold);
  console.log(
    '🔍 [FUZZY SEARCH] Itens no estoque:',
    stockItems.map(i => i.nome),
  );
  ...
  matchesWithQuality.forEach((match, index) => {
    console.log(
      `  ${index + 1}. "${match.item.nome}" - Similaridade: ${(
        match.similarity * 100
      ).toFixed(2)}% - Qualidade: ${(match.quality * 100).toFixed(2)}%`,
    );
  });
